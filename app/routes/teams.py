...
export default function Team() {
  ...
  async function downloadReport() {
    ...
    if (idsToExport.length === 0) { setError('No candidates to export'); return; }
    setDownloading(true);
    try {
              ...
              <div className="space-y-2">
                {teams.map(team => (
                  <button key={team.id} onClick={() => setSelectedTeam(team)}
                    className={'w-full text-left px-3 py-2 rounded-lg text-sm ' + (selectedTeam?.id === team.id ? 'bg-blue-50 text-blue-700 font-medium' : 'hover:bg-slate-50 text-slate-700')}>
                    {team.name}
