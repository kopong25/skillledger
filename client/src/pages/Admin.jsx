      ...
      <div className="flex gap-2 mb-6 border-b border-slate-200">
        {['stats', 'users', 'teams'].map(t => (
          <button key={t} onClick={() => setTab(t)}
            className={`px-4 py-2 text-sm font-medium capitalize border-b-2 transition-colors ${tab === t ? 'border-blue-600 text-blue-600' : 'border-transparent text-slate-500 hover:text-slate-800'}`}>
            {t}
          </button>
        ))}
      </div>
            ...
            <tbody className="divide-y divide-slate-100">
              {teams.map(t => (
                <tr key={t.id} className="hover:bg-slate-50">
                  ...
                  <td className="px-4 py-3 text-slate-500">{t.owner_name}<br /><span className="text-xs text-slate-400">{t.owner_email}                           </span></td>
